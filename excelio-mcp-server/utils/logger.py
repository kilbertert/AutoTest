...
# Copied from api-mcp-server/utils/logger.py — same logging convention.
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if is_error_result:
                ...
                logger.info(f"Tool Call - Success - ID: {call_id} - Tool: {tool_name}")
            return result
        except Exception as e:
            import traceback
            traceback.print_exc()
            logger.error(f"Tool Call - Error - ID: {call_id} - Tool: {tool_name} - Error: {str(e)}")
            raise
