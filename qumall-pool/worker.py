...
def _build_runner_command(job: dict, run_id: str) -> list[str]:
    ...
    prompt = (
        ...
        f"\n"
        f"【⚠ 关键 - qumall 真实 URL = {config.QUMALL_TARGET_URL}】\n"
        f"chrome-devtools-mcp 启动 Edge 后默认打开的标签页是 http://localhost:30081/ ，"
        f"那是 chrome-devtools-mcp 自己的 CDP 通信端口，不是 qumall 后台！\n"
        f"**严禁**把那个标签页当成 qumall 站点去操作！\n"
        f"**第一次 chrome-devtools 操作必须是** navigate_page(\"{config.QUMALL_TARGET_URL}\")，"
        f"然后从该 URL 的菜单进入\"{module}\"模块。\n"
        f"\n"
        f"【路径 - 用环境变量，绝对不要自己拼】\n"
        f"数据库路径已设到环境变量 POOL_DB，必须用 $POOL_DB。例：\n"
        f"  uv run python qumall-db/import_xlsx.py --db \"$POOL_DB\" --queue \"$POOL_QUEUE\"\n"
        ...
        f"严禁 import_xlsx.py 加 --reset（会清空其他机器的成果）。\n"
        f"\n"
        f"【硬约束】\n"
        ...
        f"- 严禁 navigate 到 http://localhost:30081/ 或 192.168.x.x:30081/（那是 chrome-devtools-mcp 占位页）\n"
        f"\n"
        f"【故障恢复】\n"
        ...
        f"\n"
        f"【流程】\n"
        f"1) uv run python qumall-db/import_xlsx.py --db \"$POOL_DB\" --queue \"$POOL_QUEUE\"\n"
        f"   （**严禁加 --reset**：会清空其他 worker 已经写好的 pass/fail 结果）\n"
        ...
        f"4) 循环 next-pending → navigate_page({config.QUMALL_TARGET_URL}) → 进模块 → set + update_cells → report_progress\n"
