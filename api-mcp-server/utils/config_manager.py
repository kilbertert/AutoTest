...
class ConfigManager:
    """Manages configuration loading and hot-reloading for API testing"""
        ...
        try:
            with self._lock:
                ...
                if not os.path.exists(self.config_path):
                    # If config file doesn't exist, use template
                    template_path = os.path.join(
                        os.path.dirname(self.config_path),
                        "api_conf.template.json"
                    )
                    if os.path.exists(template_path):
                        logger.info(f"Config file not found, using template: {template_path}")
                ...
                logger.info(f"Configuration loaded successfully. base_url: {config_data.get('base_url', 'not set')}")
